...
class TestSalesReport(unittest.TestCase):
    ...
    def test_join_monthly_data(self): # test the  jointed and the saved data
        jointed = (Report1.get_joined_monthly_data()).reset_index(drop=True)
        saved = (pd.read_csv(os.path.join(op_path, new_name_file + '.csv'))).reset_index(drop=True)
        assert_frame_equal(jointed.reset_index(drop=True), saved.reset_index(drop=True))
 
    def test_clean_empty_val(self):
        cleaned = Report1.clean_empty_val()
